...
console.log(`Server basePath: ${basePath}`); // 確認のため出力

app.prepare().then(() => {
  const server = express();
  const httpServer = http.createServer(server);

  // basePath をミドルウェアとして設定
  if (basePath) {
    server.use(basePath, express.static('public'));
  }

  // Socket.IOの初期化
  const io = initIO(httpServer);

  // Socket.IO のパスは utils/socket.js で設定済み
  // 例: path: `${basePath}/socket.io`

  // JSONボディのパースを有効化
  server.use(express.json());

  // APIルートの定義を basePath に追加
  if (basePath) {
    server.get(`${basePath}/api/group/:id/candidates`, async (req, res) => {
      const { id } = req.params;
      try {
    ...
    server.post(`${basePath}/api/group/:id/candidates`, async (req, res) => {
      const { id } = req.params;
    ...
    server.delete(`${basePath}/api/group/:id/candidates`, async (req, res) => {
      const { id } = req.params;
    ...
    // コメント追加用のルートを basePath に追加
    server.post(`${basePath}/api/group/:id/comments`, async (req, res) => {
      const { id } = req.params;
