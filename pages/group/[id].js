...
const generateICSContent = (results, groupName) => {
  // This function is used in handleDownload
...
export async function getServerSideProps(context) {
  ...
  const baseUrl = `${protocol}://${host}${basePath}`; // basePath を追加
  
  try {
    const res = await fetch(`${baseUrl}/api/group/${id}`); // 修正
...
// Removed unused variable 'socket'

export default function GroupPage({ version, groupName: initialGroupName }) {
  ...
  useEffect(() => {
    if (id) {
      fetch(getApiUrl(`/api/group/${id}`)) // basePath を含まない
      .then((res) => res.json())
      .then((data) => {
        if (data.name) {
  ...
  useEffect(() => {
    if (id) {
      fetch(getApiUrl(`/api/group/${id}/candidates`)) // basePath を含まない
      .then(res => res.json())
      .then(data => {
  ...
  useEffect(() => {
    if (id) {
      fetch(getApiUrl(`/api/group/${id}/results`)) // basePath を含まない
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data)) {
          setResults(data);
  ...
  const handleDateClick = async (clickInfo) => {
    ...
    if (existingEvent) {
      ...
      try {
        ...
        const res = await fetch(getApiUrl(`/api/group/${id}/candidates`), {
          method: 'DELETE',
  ...
  return (
    <Container className="mt-5">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
        <a href={`${basePath}/`} style={{ display: 'flex', alignItems: 'center', textDecoration: 'none' }}>
          <img 
            src={`${basePath}/meeting-scheduler.png`} 
